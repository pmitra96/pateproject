...
func TestParseDeterministicCRUD_DoesNotLogQuestion(t *testing.T) {
	msg := "what should i have for lunch"
	_, ok := parseDeterministicCRUD(msg)
	if ok {
		t.Fatalf("question should not be parsed as deterministic meal log")
	}
}

func TestParseDeterministicCRUD_DoesNotParseNutritionQuestion(t *testing.T) {
	msg := "how many calories in 1 carrot?"
	_, ok := parseDeterministicCRUD(msg)
	if ok {
		t.Fatalf("nutrition question should not be parsed as deterministic meal log")
	}
}

func TestParseDeterministicCRUD_CleansAddToMealSuffix(t *testing.T) {
	msg := "add sour dough bread to breakfast as well"
	decision, ok := parseDeterministicCRUD(msg)
	if !ok {
		t.Fatalf("expected deterministic parse")
	}
	...
	if meals[0]["ingredients"] != "sour dough bread" {
		t.Fatalf("unexpected cleaned ingredients: %#v", meals[0]["ingredients"])
	}
	if meals[0]["dish_name"] != "Sour Dough Bread" {
