...
func main() {
	...
	for i, row := range rows {
		...
		if strings.Contains(cleanRowText, "description") || strings.Contains(cleanRowText, "qty") ||
			strings.Contains(cleanRowText, "item") || strings.Contains(cleanRowText, "quantity") {
			fmt.Printf("Row %d (Y=%.2f):\n", i, row.y)
			fmt.Printf("  Raw: %s\n", rowText)
			fmt.Printf("  Clean: %s\n", cleanRowText)
			fmt.Printf("  Has 'description': %v\n", strings.Contains(cleanRowText, "description"))
			fmt.Printf("  Has 'item': %v\n", strings.Contains(cleanRowText, "item"))
			fmt.Printf("  Has 'qty': %v\n", strings.Contains(cleanRowText, "qty"))
			fmt.Printf("  Has 'quantity': %v\n\n", strings.Contains(cleanRowText, "quantity"))
		}
	}
}
